...
# Есть два списка (то есть две команды), по 20 участников в каждом. В этих списках хранятся очки каждого участника
# (это вещественные числа с двумя знаками после точки, например 4.03). Участник одной команды соревнуется с участником
# другой команды под таким же номером. То есть первый соревнуется с первым, второй — со вторым и так далее.
...
import random
f_lst = [round(random.uniform(0,10),2) for x in range(10)]
s_lst = [round(random.uniform(0,10),2) for _ in range(10)]
result = [f_lst[idx] if f_lst[idx] > s_lst[idx] else s_lst[idx] for idx in range(10)]
print(f_lst)
print(s_lst)
print(result)
