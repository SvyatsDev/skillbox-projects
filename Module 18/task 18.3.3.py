...
#
celebration = input('Введите поздравление в формате {name} and {age}: ')
name = input('Введите имена именинников через запятую: ').split(', ')
age = input('введите возраст людей через пробел: ').split()
for _ in range(len(name)):
    print(celebration.format(
        name=name[_],
        age=age[_]
    ))
people = [' '.join([name[i], age[i]]) for i in range(len(name))]
